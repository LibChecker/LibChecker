...
class PredicateStatisticChartDataSource(
  ...
  override suspend fun fillChartView(chartView: PieChart, onProgressUpdated: (Int) -> Unit) {
    withContext(Dispatchers.Default) {
      ...
      val data = buildData(items, predicate) { progress ->
        withContext(Dispatchers.Main) {
          onProgressUpdated(progress)
        }
      } ?: return@withContext
      val labels = listOf(
        predicate.matchedTitle.resolve(context),
        predicate.unmatchedTitle.resolve(context)
      )
  ...
  override fun getLabelByXValue(context: Context, x: Int): String {
    return when (x) {
      MATCHED -> predicate.matchedTitle.resolve(context)
      UNMATCHED -> predicate.unmatchedTitle.resolve(context)
      else -> ""
    }
  }
